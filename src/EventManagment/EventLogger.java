...
public class EventLogger {
    ...
    public boolean createNewEvent(SEventType eventType, SEventSeveriry eventSeverity, int source, int eventTextId) {

        switch (eventType.getEventTypeId()) {
            case 0:
    ...
    protected DeviceEvent createDeviceEvent(SEventSeveriry eventSeverity, int deviceId, int eventTextId) {
        int num = eventLog.size();
        return new DeviceEvent(num, eventSeverity, deviceId, eventTexts.get(eventTextId));

    }

    protected AppEvent createAppEvent(SEventSeveriry eventSeverity, int someId, int eventTextId) {
        int num = eventLog.size();
